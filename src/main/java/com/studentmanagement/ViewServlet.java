...
public class ViewServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException {
		...
		if(hs!=null) {
		pw.println("Welcome user:  "+(String)hs.getAttribute("name"));
		pw.println("<body>");
		pw.println("<table border='1px'>");
		pw.println("<tr><th>name</th><th>phone</th><th>email</th><th>address</th><th>course</th>");
		al.forEach(al1->{
			pw.println("<tr><td>"+al1.getName()+"</td><td>"+al1.getPhone()+"</td><td>"
		+al1.getEmail()+"</td><td>"+al1.getAddress()+"</td><td>"+al1.getCourse()+"</td></tr>");
		});
		pw.println("</table>");
		pw.println("</body>");
